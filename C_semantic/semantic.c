...
BOOLEAN isNotSameType(A_TYPE *,A_TYPE *);//추가
A_LITERAL checkTypeAndConvertLiteral(A_LITERAL,A_TYPE *,int);
A_LITERAL getTypeAndValueOfExpression(A_NODE *);
A_TYPE *setTypeElementType(A_TYPE *,A_TYPE *);//추가
A_TYPE *makeType(T_KIND);//추가
void setTypeSize(A_TYPE *,int);//추가
void semantic_error(int i, int ll, char *s);
void semantic_warning(int,int);
A_NODE *makeNode(NODE_NAME,A_NODE *,A_NODE *,A_NODE *);//추가
...
// 원시 프로그램에서 선언된 전역변수의 크기계산
void sem_program(A_NODE *node){
    int i;
    switch(node->name){
        case N_PROGRAM:
            i=sem_declaration_list(node->clink,12); //first variable 주소 : 12
            node->value=global_address; // 모든 전역변수의 크기 저장
            break;
        default:
...
// 수식을 분석, 필요한 경우 변환, 수식의 타입 계산하여 그 노드에 저장하고 리턴
A_TYPE *sem_expression(A_NODE *node){
    ...
    switch (node->name){
        ...
        case N_EXP_FLOAT_CONST:
            lit.type=float_type;
			lit.value.f=(float)atof(node->clink); //double이라서 float으로 캐스팅 해줘야함
...
// 함수 호출 명령문을 분석 중 파라미터의 개수와 타입이 일치하는지 분석
	...
	switch (node->name) {
		case N_ARG_LIST :
			if (id==0) {
                ...
                else {// DOTDOTDOT parameter: no conversion
					t=sem_expression(node->llink);
...
// 명령문 분석, (복합 명령문의 경우) 명령문들에 나타난 지역 변수들의 크기를 계산/리턴
		...
		case N_STMT_RETURN:
			if(node->clink){
				...
				if (isAllowableCastingConversion(ret,t))
					node->clink=
						convertCastingConversion(node->clink,ret);}
			else
				semantic_error(59,node->line,"");
			break;
	}
...
//타입 테이블 분석, 타입의 크기 계산/저장, 크기 값 리턴
int sem_A_TYPE(A_TYPE *t){
	...
	if (t->check)	//already checked type
		return (t->size);
	t->check=1;		//루프 안돌게하기(linked list에서 포인터 만들때)
	switch (t->kind) {
        case T_NULL:
		...
		case T_STRUCT:
			...
			id=t->field;
			while (id) {
				//20182601 크기가 0이되면 error. 더하기 전에 값을 하나 취해서 에러메시지 출력
...
// 명령문들 분석, 각 명령문들에 나타난 지역변수들의 크기 계산하여 그들 중 최대 값을 리턴
...
// 선언문에 나타난 각 지역 변수의 주소를 설정, 크기 계산, 크기값 리턴
int sem_declaration(A_ID *id,int addr){
	...
	switch (id->kind) {
		...
		case ID_PARM:
			if (id->type) {
				size = sem_A_TYPE(id->type);// usual unary conversion
...
A_LITERAL getTypeAndValueOfExpression(A_NODE *node){
    ...
    switch(node->name) {
		...
		case N_EXP_FLOAT_CONST :
			result.type=float_type;
			result.value.f=(float)atof(node->clink); //double이라서 float으로 캐스팅 해줘야함
			break;
		case N_EXP_STRING_LITERAL :
		case N_EXP_ARRAY :
		case N_EXP_FUNCTION_CALL :
...
void setTypeSize(A_TYPE *t,int ll){//추가
